...
namespace CompulsoryCat.Shell
{
    #region Shortcut Object

    /// <summary>This class contains the code for talking COM and working with a shortcut.
    /// This code is copied from http://www.vbaccelerator.com/home/net/code/libraries/shell_projects/Creating_and_Modifying_Shortcuts/ShellLink_Code.asp
    /// which was referenced from  http://www.vbaccelerator.com/home/net/code/libraries/shell_projects/Creating_and_Modifying_Shortcuts/article.asp
    /// I didn't find any license so I dare a guess that including it in a LGPL project would be ok.
    /// Big thanks to Steve McMahon steve(at)vbaccelerator.com
    /// </summary>
    public class Shortcut : IDisposable
    {
        ...
        /// <summary>The standard suffix.
        /// Can this be taken from the system?  Like Environment.ShortcutSuffix?
        /// </summary>
        public const string Suffix = "lnk";
        ...
        /// <summary>This is an enumeration of how to show whatever is opened from the shortcut.
        /// </summary>
        public enum LinkDisplayMode : uint
        {
            /// <summary>Open the target as normal (not minimized or maximized).
            /// </summary>
            edmNormal = EShowWindowFlags.SW_NORMAL,
            /// <summary>Open the target as minimized.
            /// </summary>
            edmMinimized = EShowWindowFlags.SW_SHOWMINNOACTIVE,
            /// <summary>Open the target in maximized mode.
            /// </summary>
        ...
        public Shortcut(string linkFile)
            : this()
        {
            Open(linkFile);
        }
        ...
        /// <summary>The (path and) name of the "physical file' of the shortcut.
        /// </summary>
        ...
        /// <summary>Getter for the small icon for the shortcut.
        /// </summary>
        ...
        /// <summary>This method deletes all Shortcuts in the path with 
        /// the target targetPathFile.
        /// There is no folder recursion.
        /// </summary>
        /// <param name="path">The path looked in.</param>
        /// <param name="targetPathFile">The target, path and all.</param>
        public static void DeleteForTarget(string path, string target)
        {
            var directoryInfo = new System.IO.DirectoryInfo(path);
            var fileArray = directoryInfo.GetFiles("*.lnk");
            var targetPathFile = new List<string>();
            foreach (var file in fileArray)
            {
                using (var shortcut = new Shortcut())
                {
                    shortcut.Open(file.FullName);
                    if (shortcut.Target.ToLower() == target.ToLower())
                    {
                        //  Add to the list.  We don't want to delete files we might have open
                        //  or anything we iterate.
                        targetPathFile.Add(file.FullName);
                    }
                }
            }
            foreach (var filePathName in targetPathFile)
            {
